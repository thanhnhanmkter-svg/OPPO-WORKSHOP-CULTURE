...
function JoinPopup({ name, dept, onDone }) {
  ...
  return (
    <div className="animate-slideIn flex items-center gap-3 bg-emerald-500/20 border border-emerald-500/40 rounded-2xl px-5 py-3 shadow-xl">
      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-green-400 flex items-center justify-center text-white font-black text-lg flex-shrink-0">
        {name.charAt(0).toUpperCase()}
      </div>
      <div>
        <p className="text-white font-bold text-base">{name}</p>
        {dept && <p className="text-emerald-400 text-xs">{dept}</p>}
      </div>
...
// Stage 0: Welcome + QR + danh sách + nút bắt đầu
function HostWelcome({ url, users, onStart }) {
  ...
  useEffect(() => {
    if (!url) return;
    QRCode.toDataURL(url, { width: 280, margin: 2, color: { dark: "#ffffff", light: "#00000000" } })
      .then(setQr).catch(console.error);
  }, [url]);

  // Hiện popup khi có người mới join
  useEffect(() => {
    if (users.length > prevUserCount.current) {
      const newUsers = users.slice(prevUserCount.current);
      newUsers.forEach((u) => {
  ...
  function removePopup(id) { setPopups((p) => p.filter((x) => x.id !== id)); }
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-8">
        ...
        <button
          ...
          className="px-14 py-5 rounded-2xl bg-gradient-to-r from-emerald-500 to-green-400 text-white font-black text-2xl
            hover:scale-105 active:scale-95 transition-all shadow-xl shadow-emerald-500/40
      ...
      {/* Right: Danh sách người tham gia */}
      <div className="w-80 flex flex-col border-l border-white/10 p-6 gap-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {users.length === 0 && (
            <p className="text-gray-600 text-sm text-center mt-8">Chưa có ai tham gia…</p>
          )}
          {users.map((u) => (
            <div key={u.uid} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-green-400 flex items-center justify-center text-white font-black text-sm flex-shrink-0">
                {u.name?.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="text-white text-sm font-semibold truncate">{u.name}</p>
                {u.dept && <p className="text-gray-500 text-xs truncate">{u.dept}</p>}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-8">
      <div className="text-center">
        ...
        <h2 className="text-9xl font-black text-white">❤️</h2>
        <p className="text-8xl font-black text-rose-400 mt-2">{totalHearts}</p>
        <p className="text-gray-400 text-2xl mt-2">tim đã gửi</p>
      </div>
      <div className="flex flex-wrap justify-center gap-3 max-w-3xl px-8">
        {users.map((u) => (
...
// Stage 2: Case Study
function HostCaseStudy({ sessionData, answers, onShowAnswer, onNext }) {
  ...
  if (!q) return (
    <div className="flex items-center justify-center h-full">
      <p className="text-gray-400 text-2xl">Chưa có câu hỏi. Vào Admin → Case Study để thêm.</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full p-10 gap-6">
      ...
      <div className="flex gap-4 justify-center items-center">
        ...
        <span className="text-gray-500 text-sm">{totalVotes} người đã chọn</span>
      </div>
    </div>
  );
}
...
// Stage 3: Sharing
function HostSharing({ totalHearts }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-8">
      <div className="text-center">
  ...
  if (showEnd) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-6 text-center">
        ...
        <h1 className="text-9xl font-black text-emerald-400 leading-tight">CHÀO MỪNG</h1>
...
export default function HostPage() {
  ...
  useEffect(() => {
    ...
    const u5 = onValue(ref(db, "scores"), (snap) => {
      ...
      const list = Object.entries(snap.val())
        .filter(([_, s]) => s.name)
        .map(([uid, s]) => ({ uid, name: s.name, total: (s.quiz || 0) + (s.hearts || 0) }))
  ...
  async function handleShowAnswer() { await set(ref(db, "session/showAnswer"), true); }
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-emerald-950 relative overflow-hidden" style={{ height: "100vh" }}>
        ...
        <div className="h-full">
          ...
          {stage === 3 && <HostSharing totalHearts={totalHearts} />}
